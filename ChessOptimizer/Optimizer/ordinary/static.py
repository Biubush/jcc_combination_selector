...
def main():
    ...
    # 找到最佳组合
    max_synergy, best_combo, best_synergy_list, best_total_cost = find_max_synergy(
    ...
    output_file = os.path.join(
        output_folder, f"{budget_limit}g_{people_limit}p.txt"
    )  # 输出文件路径

    # 将结果写入文件
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(
            f"最大羁绊数: {max_synergy}\n最佳棋子组合: {', '.join(best_combo)}\n激活的羁绊: {', '.join(best_synergy_list)}\n总费用: {best_total_cost}"
        )
